...
const urlRegioner = "https://api.dataforsyningen.dk/kommuner"
const urlPostRegion = "http://localhost:8080/kommune" //postmappingens endpoint, så fetch kan kalde det
...
kommuneList = [] //så den er global og kan bruges senere
async function showAllRegioner() {
    kommuneList = await fetchAny(urlRegioner); //regionliste er en liste hvis man får liste tilbage,
    //og objekt hvis man kun får et enkelt objekt tilbage
    console.log(kommuneList)
    kommuneList.forEach(fillRegionDropDown)
}
...
pbFetchRegioner.addEventListener('click', showAllRegioner) //når async funktionen skal kaldes via eventlistener så skal der være en
//metode imellem, eventlistener skal kalde noget som så kalder async, det vil erik have, men det betyder ikke noget
...
function actionPostAllRegioner(){  //erik bruger action på trykknap-funktioner
    ...
    if (kommuneList){ //erik gætter, tomt array er false og fyldt array er true
        console.log("post alle regioner")
        kommuneList.forEach(postRegioner)
